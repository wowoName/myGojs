        ...
        init() {
            ...
            this.myDiagram.linkTemplate =
                goMake(go.Link,//MultiArrowLink Link
                    { toShortLength: 3 },
                    new go.Binding("selectable", "category", value => {
                        console.log(value === 'none' ? true : false)
                        return value === 'none' ? true : false
                    }),
                    go.Link.Orthogonal,//连线样式 go.Link.Bezier  Orthogonal go.Link.Orthogonal,
                    //连接线颜色设置
                    goMake(go.Shape, { strokeWidth: 3, }, new go.Binding("stroke", "color")),

                    // 箭头样式设置
                    goMake(go.Shape, { toArrow: "Standard", stroke: null, strokeWidth: 3, scale: 1 }
