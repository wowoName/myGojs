                            ...
                            goMake(SwimLaneLayout,
                                {
                                    ...
                                    commitLayers: function (layerRects, offset) {
                                        ...
                                        let offsetX = 0, offsetWidth = 0;
                                        for (let i = 0; i < this.laneNames.length; i++) {
                                            let lane = this.laneNames[i];
                                            //将大组之间分隔开
                                            if (lane.endsWith('1')) offsetX = 100, offsetWidth = 50;
                                            else offsetX = 0, offsetWidth = 0;
                                            ...
                                            if (horiz) {
                                                ...
                                                group.location = new go.Point(this.lanePositions.get(lane) * this.columnSpacing + offset.x + offsetX / 2, -this.layerSpacing / 2);
                                            }
                                            ...
                                            if (horiz) {
                                                ...
                                                ph.desiredSize = new go.Size(this.laneBreadths.get(lane) * this.columnSpacing - offsetWidth, totallength);
                                            }
                                        }
                                    }
                                })
                ...
                goMake(go.Node, go.Panel.Auto,
                    ...
                    goMake(go.Panel, "Vertical",  // the whole node panel
                        ...
                        new Binding("margin", "type", value => {
                            return value === 'information' ? new go.Margin(0, 0, 0, 50) : new go.Margin(0, 0, 0, 180)
                        }),
            ...
            */
            ...
            myDiagram.groupTemplate = goMake(go.Group, (DIRECTION === 0 || DIRECTION === 180) ? "Horizontal" : "Vertical",
                ...
                goMake(go.Panel, "Auto", {
                    ...
                    goMake(go.Shape, { fill: "transparent", stroke: "#00519c" },
                        new Binding("margin", "type", value => {
                            returnvalue && value === "command" ? 0 : 0
                        })
                    ),
                    goMake(go.Shape, { name: "PLACEHOLDER", fill: null, stroke: null, strokeWidth: 1, margin: 0 },
                        //  new Binding("margin", "type", value => {
                        //         return value === "command" ? new go.Margin(0, -100, 0, 0) : new go.Margin(0, 0, 0, 0)
                        //     })
                    )
                )
